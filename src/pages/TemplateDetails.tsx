...
export const TemplateDetails = () => {
  ...
  const loadTemplate = async () => {
    ...
    try {
      setError(null);
      // Load template with creator info
      const { data: templateData, error: templateError } = await supabase
        .from('templates')
        .select(`
          *,
          ratings(*),
          creator:profiles!templates_user_id_fkey (
            username,
            avatar_url
          )
        `)
      ...
      // Set creator info from joined profiles data
      if (templateData.creator) {
        setCreator({
          username: templateData.creator.username,
          avatarUrl: templateData.creator.avatar_url,
        });
      }
      ...
      setError('Error al cargar la plantilla. Por favor, intente de nuevo.');
    } finally {
      setIsLoading(false);
    }
  };
